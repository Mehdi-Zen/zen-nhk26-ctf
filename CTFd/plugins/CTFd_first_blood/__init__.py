...
# Event listeners disabled - these imports kept for potential future use
...
class FirstBloodChallenge(DynamicChallenge):
...
class FirstBloodValueChallenge(DynamicValueChallenge):
    ...
    def read(cls, challenge):
        ...
        """
        challenge = FirstBloodChallenge.query.filter_by(id=challenge.id).first()
    ...
    def update(cls, challenge, request):
        ...
        for attr, value in data.items():
            if attr.startswith('first_blood_bonus'):
                first_blood_bonus = []
                continue
            # Handle dynamic fields - convert to float
    ...
    def solve(cls, user, team, challenge, request):
        ...
        if solve and FirstBloodValueChallenge._can_get_award(challenge, solve):
...
# NOTE: Event listeners disabled because they interfere with challenge deletion in CTFd 3.x
# The first blood awards will still be created when challenges are solved,
# but they won't be automatically cleaned up when solves/users/teams are deleted.
# This is an acceptable tradeoff for stability.
