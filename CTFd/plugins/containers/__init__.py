...
from CTFd.models import db, Solves, Teams, Users, Awards, Tags
...
with open(get_settings_path()) as _f:
...
class ContainerChallenge(BaseChallenge):
    ...
    def solve(cls, user, team, challenge, request):
        ...
        # First Blood bonus for container challenges with FIRST BLOOD tag
        try:
            challenge_tags = Tags.query.filter_by(challenge_id=challenge.id).all()
            has_firstblood_tag = any(
                t.value.upper() in ('FIRST BLOOD', 'FIRSTBLOOD', 'FIRST_BLOOD')
                for t in challenge_tags
            )

            if has_firstblood_tag:
                ...
                if solve_count <= 1:  # First solve
                    account_id = team.id if team else user.id
                    account_name = team.name if team else user.name
                    bonus = int(challenge.value * 0.5)  # 50% bonus
                    award = Awards(
                        user_id=user.id,
                        team_id=team.id if team else None,
                        name=f"First Blood: {challenge.name}",
                        description=f"Premier sang sur {challenge.name} ! +{bonus} points",
                        value=bonus,
                        category=challenge.category,
                    )
                    ...
                    # Emit SSE event for firstblood animation
                    first_blood_data = {
                    ...
                    current_app.events_manager.publish(
                        data=first_blood_data, type="first_blood", channel="ctf"
                    )
        except Exception:
            pass
