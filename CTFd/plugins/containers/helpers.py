...
def create_container(container_manager, chal_id, xid, is_team):
    ...
    max_containers_val = container_manager.settings.get("max_containers", 3)
    max_containers = int(max_containers_val) if max_containers_val not in (None, "") else 3
    ...
    if running_container:
        try:
            if container_manager.is_container_running(running_container.container_id):
                ...
                db.session.delete(running_container)
                db.session.commit()
                return jsonify({"status": "Container stopped, please start a new one."})
        except ContainerException as err:
            return jsonify({"error": str(err)}), 500
    else:
