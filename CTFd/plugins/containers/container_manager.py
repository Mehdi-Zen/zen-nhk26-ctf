...
class ContainerManager:
    ...
    def _reconnect(self):
        """Try to reconnect to Docker with backoff. Does NOT create new schedulers."""
        if not hasattr(self, '_reconnect_attempts'):
            self._reconnect_attempts = 0
            self._last_reconnect = 0
        if not hasattr(self, 'expiration_seconds'):
            try:
        ...
        now = time.time()
        # Backoff: wait at least 2^attempts seconds (max 60s) between reconnects
        wait_time = min(60, 2 ** self._reconnect_attempts)
        if now - self._last_reconnect < wait_time:
            return False
        ...
        try:
            ...
            self.client = docker.DockerClient(base_url=self.settings.get("docker_base_url"))
            self._reconnect_attempts = 0  # Reset on success
            print("[ContainerManager] Reconnected to Docker successfully")
            return True
        except Exception as e:
            self._reconnect_attempts = min(self._reconnect_attempts + 1, 6)  # Cap at 64s
            self.client = None
            return False
    ...
    def run_command(func):
        def wrapper_run_command(self, *args, **kwargs):
            ...
            try:
                ...
                print(f"[ContainerManager] Error: {type(e).__name__}: {e}")
                self._reconnect()
                raise ContainerException(
    ...
    def create_container(self, challenge, xid, is_team):
        ...
        if self.settings.get("container_maxmemory") not in (None, ""):
            try:
        ...
        if self.settings.get("container_maxcpu") not in (None, ""):
            try:
