        ...
        if not solves:
            ...
            if status == "correct" or status is True:
                ...
                if ctftime() or current_user.is_admin():
                    # Check if this is a first blood before solving
                    existing_solves = Solves.query.filter_by(challenge_id=challenge_id).count()
                    is_first_blood = (existing_solves == 0)
                    ...
                    # Publish first blood event only for firstblood type challenges
                    if is_first_blood and challenge.type == "firstblood":
                        # Get the account name (team name if in team mode, user name otherwise)
                        if get_config("user_mode") == "teams" and team:
